...
public class MultiLabelling
{
	...
	{
		Console.WriteLine ("=========================================");
		Console.WriteLine ("Define the truncation (between 0.01 and 1, leave it to 0.01 if not needed)");
		...
		if (!float.TryParse (Console.ReadLine (), out temptruncated)) 
		{
			...
			while (true) 
			{
				...
				Math.Round (temptruncated, 2);
			}
		}
		//check that it has the 2 decimal places required and it should not be 0
	...
	void DefineOccluded()
	{
		...
		if (tempoccluded > 3 || tempoccluded < 0) 
		{
			...
			//ask the user again
			string error;
			error = "Error: Input is not between 0 and 3. \n";
	...
	{
		Console.WriteLine ("=========================================");
		Console.WriteLine ("Define the observation angle (between -pi and pi, leave it to 0.01 if not needed)");
